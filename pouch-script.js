...
if ('serviceWorker' in navigator) {
  ...
  // Флаг для предотвращения повторного показа уведомления
    ...
    .then(registration => {
      ...
      registration.addEventListener('updatefound', () => {
        ...
        newWorker.addEventListener('statechange', () => {
          ...
          if (newWorker.state === 'installed' && navigator.serviceWorker.controller && !isUpdateNotified) {
            console.log('Новая версия Service Worker установлена, показываем уведомление');
            isUpdateNotified = true; // Устанавливаем флаг

            const updateMessage = document.createElement('div');
            updateMessage.id = 'update-message'; // Добавляем ID для управления
  ...
  navigator.serviceWorker.addEventListener('controllerchange', () => {
    ...
    // Удаляем уведомление перед перезагрузкой
