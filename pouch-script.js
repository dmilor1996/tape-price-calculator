...
firebase.firestore().enablePersistence()
    ...
    .catch(error => {
        console.error("Ошибка включения оффлайн-поддержки:", error);
    });
...
// Попробуем загрузить последние данные из localStorage
function loadFromLocalStorage() {
    const storedPrices = localStorage.getItem('pouchPrices');
    if (storedPrices) {
        pouchPrices = JSON.parse(storedPrices);
        console.log("Цены загружены из localStorage:", pouchPrices);
    }
}

// Сохраняем цены в localStorage
...
async function loadPouchPrices() {
    try {
        ...
        // Если есть ошибка и мы оффлайн, используем localStorage
        if (!navigator.onLine) {
            loadFromLocalStorage();
            if (Object.keys(pouchPrices).length > 0) {
                document.getElementById("result").innerText = "Работа в оффлайн-режиме: используются последние сохранённые данные.";
            } else {
                document.getElementById("result").innerText = "Ошибка: нет сохранённых данных для оффлайн-режима.";
            }
        } else {
